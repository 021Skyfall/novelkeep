...
- `UNIQUE(member_type)`으로 역할별 최대 1명을 보장하고, 정확히 1명은 최초 역할 선택 시 지연 생성으로 맞춘다.
- 역할별 고정 체험 회원을 최초 1회 생성하고 이후 역할 선택에서는 같은 회원을 재사용한다. 재접속 시 기존 작품·펀딩·주문을 같은 소유자로 조회하기 위해서다.
